    ...
    if @cache.length > 0
      ...
      if @cache.length > 0
        trigger = =>
          ...
          @robot.reply reminder.msg_envelope, 'リマインドです！確認してください！ ' + reminder.action
          @queue()
...
module.exports = (robot) ->
  ...
  robot.respond /リマインド ((?:(?:\d+) (?:weeks?|days?|hours?|hrs?|minutes?|mins?|秒?|secs?)[ ,]*(?:and)? +)+)to (.*)/i, (msg) ->
    time = msg.match[1]
    action = msg.match[2]
    reminder = new Reminder msg.envelope, time, action
    reminders.add reminder
   # msg.send 'I\'リマインドします！ ' + action + ' on ' + reminder.dueDate()
    msg.send 'はい！リマインドします！ ' + action + ' on ' + reminder.dueDate()
